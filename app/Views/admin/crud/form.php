      ...
      <div class="form-grid">
        ...
        ?>
          <div class="form-field <?= $full ? 'full' : '' ?>">
            <label for="field-<?= e($name) ?>"><?= e($f['label']) ?><?= !empty($f['rules']) && str_contains($f['rules'], 'required') ? ' *' : '' ?></label>
            <?php if (!empty($f['hint'])): ?><span class="hint"><?= e($f['hint']) ?></span><?php endif; ?>

            <?php if ($type === 'textarea' || $type === 'richtext'): ?>
              <textarea id="field-<?= e($name) ?>" name="<?= e($name) ?>" rows="<?= $type === 'richtext' ? 12 : 4 ?>" class="<?= $invalid ?>"<?= $type === 'richtext' ? ' data-richtext' : '' ?> placeholder="<?= e($f['placeholder'] ?? '') ?>"><?= e($value) ?></textarea>

            <?php elseif ($type === 'json'): ?>
              <textarea id="field-<?= e($name) ?>" name="<?= e($name) ?>" rows="6" class="mono <?= $invalid ?>" placeholder='<?= e($f['placeholder'] ?? '["item one","item two"]') ?>'><?= e($value) ?></textarea>

            <?php elseif ($type === 'country'):
                $countries = require BASE_PATH . '/app/Support/countries.php';
                $tzTarget = $f['tz_target'] ?? 'timezone';
            ?>
              <select id="field-<?= e($name) ?>" name="<?= e($name) ?>" class="<?= $invalid ?>"
            ...
            <?php elseif ($type === 'checkbox'): ?>
              <div class="check-row">
                <input id="field-<?= e($name) ?>" type="checkbox" name="<?= e($name) ?>" value="1" <?= $value ? 'checked' : '' ?>>
                <span class="muted"><?= e($f['checkbox_label'] ?? 'Enabled') ?></span>
              </div>

            <?php elseif ($type === 'date'): ?>
              <input id="field-<?= e($name) ?>" type="date" name="<?= e($name) ?>" value="<?= e($value ? date('Y-m-d', strtotime((string)$value)) : '') ?>" class="<?= $invalid ?>">

            <?php elseif ($type === 'number'): ?>
              <input id="field-<?= e($name) ?>" type="number" step="<?= e($f['step'] ?? '1') ?>" name="<?= e($name) ?>" value="<?= e($value) ?>" class="<?= $invalid ?>">
